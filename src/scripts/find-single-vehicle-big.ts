...
export default async function find({ container }: ExecArgs) {
  const db = container.resolve("__pg_connection__")
  const result = await db.raw(
    "SELECT f.vehicle_id, vm.name as make, vmod.name as model, v.year_start, v.year_end, " +
    "pc.name as sub_cat, COUNT(DISTINCT p.id) as cnt " +
    ...
    "GROUP BY f.vehicle_id, vm.name, vmod.name, v.year_start, v.year_end, pc.name " +
    "HAVING COUNT(DISTINCT p.id) > 50 " +
    "ORDER BY cnt DESC LIMIT 10"
  )
  for (const r of result.rows) {
    console.log(`${r.year_start}-${r.year_end} ${r.make} ${r.model} (${r.vehicle_id}) | ${r.sub_cat} | ${r.cnt}`)
  }
}
